<template>
                    ...
                    <div class="storeInfo">
                         <div class="storeWapper">
                            <div class="storeWapper-Item">
                                ...
                                <div class="describe" v-if="!isEditBusinessTime">未设置</div>
                                <div v-show="isEditBusinessTime">
                                    <el-form ref="form" :model="form" size="mini" label-width="70px">
                                       <div class="hours-bg" v-for="(item,index) in form.normalServingTimeList" :key="index">
                                        <el-form-item label="营业日" prop="storeName">
                                            <el-checkbox size="mini" v-for="ditem in weekList" v-model="item.weeks" :key="ditem.value" :label="ditem.name" :disabled="ditem.disabled" border>
                                           </el-checkbox>  
                                         </el-form-item>
                                         <el-form-item label="营业时段" v-for="(hItem,hindex) in item.buinessHours" :key="hindex">
                                            <el-time-picker size="mini" v-model="hItem.startTime" :clearable="false" placeholder="选择时间" format="HH:mm">
                                            </el-time-picker>
                                            <span>至</span>
                                            <el-time-picker size="mini" v-model="hItem.end" :clearable="false" placeholder="选择时间" format="HH:mm"></el-time-picker>
                                            <el-link  v-show="item.buinessHours.length>1"> <i class="el-icon-delete" @click="removeBuinessHours(index,hindex)"></i></el-link> 
                                         </el-form-item>
                                         <el-link style="padding-left:65px;" :underline="false" type="primary" icon="el-icon-plus" v-show="item.buinessHours.length === maxBuinessHours?false:true" @click="addBuinessHours(index)">新增营业时段({{ item.buinessHours.length}}/{{ maxBuinessHours }})</el-link>
                                       </div>
                                       <footer>
                                           <div style="margin-bottom:16px;">
                                            <el-link :underline="false" type="primary" icon="el-icon-plus" @click="addNormalServingTimeList" v-show="isShowAddNormalServingTimeList">新增营业日</el-link>
                                           </div>
                            ...
                            <div class="operation" v-if="!isEditBusinessTime" @click="editBusinessTime">修改</div>
                         </div>
                         <div class="storeWapper">
                            <div class="storeWapper-Item">
  ...
  <script>
  import { updateShopServingTime } from "@/api/business/store";; // 引入接口方法
    export default {
      data() {
        return {
          activeName: 'first',
      ...
      mounted(){
        this.requestStore = this.$store.state.user.user.business.store.find(t=>t.isDefault==1);
      ...
      methods: {
        ...
        addNormalServingTimeList(){
            this.form.normalServingTimeList.push({weeks:[],buinessHours:[{startTime:'',endTime:''}]})
        },
        ...
        *新增营业时段
        ...
        addBuinessHours(index){
           this.form.normalServingTimeList[index].buinessHours.push({startTime:'',endTime:''});
        },
        /***
        *删除营业时段
         */
        ...
        isDisabled() {
            ...
            for (let j = 0; j < currentServingTime.weeks.length; j++) {
                ...
                for (let l = 0; l < otherServingTime.weeks.length; l++) {
                    ...
                    if (currentWeek.value === otherWeek.value) {
        ...
        saveNormalServingTime(form){
            ...
            if (valid) {
                updateShopServingTime({params:JSON.stringify(this.form)}).then(response => {
                }).catch(error => {
...
.el-tabs{
   ...
   .el-tabs__item{font-size: 16px !important;font-weight: 500 !important;}
   .el-tabs__active-bar{
  ...
  .el-tabs__header{
    ...
    .el-tabs__nav-wrap::after{background: transparent !important;
    ...
    .storeInfo{
      ...
      .storeWapper{
        ...
        .text{width: 100px;padding-left: 15px;}
        .describe{color: #6e6f70;}
        .hours-bg{
            ...
            padding: 12px;
            position: relative;
            .el-checkbox.is-bordered.el-checkbox--mini{margin-right: 0px !important;}
    }
      }
